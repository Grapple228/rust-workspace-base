//! Model Layer
//!
//! Design:
//!
//! - The Model layer normalizes the application's data type
//!   structures and access.
//! - All application code data access must ho through the Model layer.
//! - The `ModelManager` holds the internal states/resources
//!   needed for ModelControllers to access data.
//!   (e.g., db_pool, S3 client, redis client)
//! - Model Controllers (e.g., `TaskBmc`, `ProjectBmc`) implement
//!   CRUD and other data access methods on a given "entity"
//!   (e.g. `Task`, `Project`).
//!   (`Bmc` is a short for Backend Model Controller).
//! - In frameworks like Axum, Taure, `ModelManager` are typically used as App State.
//! - ModelManager are designed to be passed as an argument
//!   to all Model Controllers functions.
//!

// region:    --- Modules

mod base;
mod error;
mod store;

use store::new_db_pool;

use crate::Db;

pub use self::error::{Error, Result};

// endregion: --- Modules

#[derive(Clone)]
pub struct ModelManager {
    db: Db,
}

impl ModelManager {
    pub async fn new() -> Result<Self> {
        let db = new_db_pool().await?;

        Ok(ModelManager { db })
    }

    /// Returns the surrea db pool reference
    /// (Only for the model layer)
    pub(in crate::model) fn db(&self) -> &Db {
        &self.db
    }
}
